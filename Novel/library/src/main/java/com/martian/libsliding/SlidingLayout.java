...
//    private long mDownMotionTime;
    ...
    public void setAdapter(SlidingAdapter adapter) {
        mDiff = (int) (getResources().getSystem().getDisplayMetrics().density * 5);
...
//        long timeDiff = System.currentTimeMillis() - mDownMotionTime;
        ...
        if (xDiff < mDiff && yDiff < mDiff) {//&& timeDiff < 200) {
    ...
    public void slideSelected() {
        if (mSlideChangeListener != null)
            mSlideChangeListener.onSlideSelected(mAdapter.getCurrent());
    }
}
